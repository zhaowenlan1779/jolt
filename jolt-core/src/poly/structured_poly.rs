...
pub struct NoPolynomialsTag {}

impl<C> StructuredCommitment<C> for NoPolynomialsTag
where
    C: CommitmentScheme,
{
    type Commitment = ();

    fn commit(&self, _generators: &C::Setup) -> Self::Commitment {
        ()
    }
}
...
pub trait StructuredOpeningProof2<F, C, Polynomials, Polynomials2 = NoPolynomialsTag>:
    Sync + CanonicalSerialize + CanonicalDeserialize
where
    F: JoltField,
    C: CommitmentScheme<Field = F>,
    Polynomials: StructuredCommitment<C> + ?Sized,
    Polynomials2: StructuredCommitment<C> + ?Sized,
{
    ...
    fn open(polynomials: &Polynomials, polynomials2: &Polynomials2, opening_point: &[F]) -> Self;
    ...
    fn verify_openings(
        ...
        commitment2: &Polynomials2::Commitment,
        opening_point: &[F],
        transcript: &mut ProofTranscript,
